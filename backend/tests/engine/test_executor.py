...
def test_join_pipeline(tmp_path):
    ...
    outputs = FlowExecutor().execute(graph, {"L": left, "R": right}, out_dir)
...
def test_concat_pipeline(tmp_path):
    ...
    outputs = FlowExecutor().execute(graph, {"A": a, "B": b}, out_dir)
    result = pd.read_csv(outputs["o"])
    assert len(result) == 4
...
def test_unknown_engine_raises(tmp_path, input_csv):
    ...
    with pytest.raises(ValueError, match="Unknown engine"):
        FlowExecutor().execute(_pipeline_graph(), {"ds1": input_csv}, out_dir, "spark")
