...
class EngineBackend(Protocol):
    ...
    # Inverse of ``to_pandas``: wrap a pandas result back into this engine's frame
    # type. ML nodes convert to pandas for sklearn, then call this so the frame
    # they hand downstream matches the active engine (Arrow-backed for polars).
    def from_pandas(self, df: pd.DataFrame) -> AnyFrame: ...
    def to_records(self, df: AnyFrame, n: int | None = None) -> list[dict[str, Any]]: ...
    def row_count(self, df: AnyFrame) -> int: ...
    # Column names without materializing the frame. Used per-node by the run DAG,
    # so it must be cheap — never convert the whole frame just to read its schema.
    def column_names(self, df: AnyFrame) -> list[str]: ...
    ...
    def fill_nulls(self, df: AnyFrame, columns: list[str] | None, strategy: str, value: Any) -> AnyFrame: ...
    def drop_nulls(self, df: AnyFrame, columns: list[str] | None, how: str = "any") -> AnyFrame: ...
    def drop_duplicates(self, df: AnyFrame, subset: list[str] | None, keep: str | bool) -> AnyFrame: ...
    def cast_column(
    ...
    def groupby_agg(self, df: AnyFrame, by: list[str], aggregations: dict[str, str]) -> AnyFrame: ...
    def join(
        self,
        left: AnyFrame,
        right: AnyFrame,
    ...
    # -- New nodes (Phase 3) -------------------------------------------
    def sample_rows(self, df: AnyFrame, n: int | None, frac: float | None, seed: int | None) -> AnyFrame: ...
    def remove_outliers(
        self,
        df: AnyFrame,
        columns: list[str],
    ...
    def extract_date_parts(self, df: AnyFrame, column: str, parts: list[str]) -> AnyFrame: ...
    def unpivot(
        self,
        df: AnyFrame,
        id_vars: list[str],
    ...
    def parse_dates(self, df: AnyFrame, columns: list[str], fmt: str | None, errors: str) -> AnyFrame: ...
    def map_values(
        self,
        df: AnyFrame,
        column: str,
