...
class ChartBarTransformation(_BaseChart):
    ...
    def validate_config(self, config: dict[str, Any]) -> None:
        ...
        if not config.get("x"):
            raise ValueError(f"{self.type} requires an 'x' (category) column.")
...
class ChartScatterTransformation(_BaseChart):
    ...
    def validate_config(self, config: dict[str, Any]) -> None:
        ...
        if not config.get("x") or not config.get("y"):
            raise ValueError(f"{self.type} requires numeric 'x' and 'y' columns.")
...
class ChartBoxPlotTransformation(_BaseChart):
    ...
    def validate_config(self, config: dict[str, Any]) -> None:
        super().validate_config(config)
        if not config.get("column"):
            raise ValueError(f"{self.type} requires a 'column' of numeric values.")
