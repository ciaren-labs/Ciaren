# FlowFrame Brand Guidelines

These guidelines help contributors, plugin authors, educators, and community
members present FlowFrame consistently without creating confusion about what is
official.

## Name

Use `FlowFrame` with capital `F`s.
...
- Plugin for FlowFrame

Avoid:

- Flowframe
- Flow Frame
- flowframe as a product name, except in package names or commands where
  lowercase is conventional

## Describing FlowFrame

Preferred short description:

> FlowFrame is a local-first visual platform for building data and ML workflows
...
use FlowFrame only to indicate compatibility.

## Official vs Community

Use clear labels:

- `Official` means published, maintained, or explicitly endorsed by the FlowFrame
  maintainers.
- `Community` means created by contributors or third parties.
- `Compatible with FlowFrame` means it works with FlowFrame, but is not
  necessarily reviewed or endorsed.

Good examples:

- `Acme Connector for FlowFrame`
- `Community template for FlowFrame`
- `Compatible with FlowFrame`

Avoid:

- `Official FlowFrame Connector` unless it is official
- `FlowFrame Cloud` for an unaffiliated hosted service
- `FlowFrame Marketplace` for a third-party marketplace unless approved
...
- Say `for FlowFrame` or `compatible with FlowFrame`.
- Include a clear disclaimer if the plugin is not official.

Example:

> Acme Snowflake Connector for FlowFrame is a third-party plugin and is not
> affiliated with or endorsed by the FlowFrame maintainers.
...
the controls they should use: local-first deployment, `FLOWFRAME_API_TOKEN`,
trusted network boundaries, representative testing, and their own review for
sensitive environments.

Avoid alarmist language. Prefer:

> FlowFrame is alpha software. For sensitive or production workflows, review the
