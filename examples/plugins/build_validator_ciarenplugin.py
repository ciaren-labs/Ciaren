#!/usr/bin/env python3
"""Build and sign the Validator plugin into a distributable ``.ciarenplugin``.

Run from anywhere (needs Ciaren installed):

    python examples/plugins/build_validator_ciarenplugin.py

It packs ``validator-plugin/`` into ``dist/community.validator-<version>.ciarenplugin``
