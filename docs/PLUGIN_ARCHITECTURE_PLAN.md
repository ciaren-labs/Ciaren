...
| 7 — Security hardening | 🟡 | Permission gating (code not imported until approved) + signature verification done; joblib/pickle refusal & env-only DB secrets already enforced; dependency-license scan (`flowframe plugin licenses`); security docs (`docs/security/`). Permission **approval UI** lands with the frontend Plugins page below. |
